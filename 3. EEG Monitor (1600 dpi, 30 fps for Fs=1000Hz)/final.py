...
def create_eeg_video(mat_file, out_file='eeg_video.mp4', window_size=6, fps=30):
    ...
    # This gives the baseline (signal_mid) somewhere between them
    ...
    # Calculate scaling to map signal range to target range
    if signal_range > 0:
    ...
    # Fixed harmonic scale values (from top to bottom)
    harmonic_scale_values = np.array([5000, 2500, 0, -2500, -5000])
    
    # Calculate where these µV values would appear in our scaled coordinate system
    # Using the inverse of our scaling transformation
    scale_positions = []
    scale_values = []
    
    for uv_value in harmonic_scale_values:
        # Calculate y position for this µV value
        if signal_range > 0:
            y_pos = target_bottom + (uv_value - signal_min) * scale_factor
        else:
            y_pos = 0.5  # Center if signal is flat
        
        # Only include values that are within the visible range
        if target_bottom - 0.1 <= y_pos <= target_top + 0.1:
            scale_positions.append(y_pos)
            scale_values.append(uv_value)
    
    scale_positions = np.array(scale_positions)
    ...
    for frame_idx in tqdm(range(total_frames)):
        ...
        # Add scale bar with adaptive values
...
def main():
    ...
    if args.all:
        ...
        for mat_file in mat_files:
            ...
            create_eeg_video(mat_file, out_file=output_video, window_size=args.window, fps=args.fps)
    
    # Single file mode
    elif args.mat_file:
        output = args.output if args.output else os.path.splitext(args.mat_file)[0] + "_video.mp4"
        create_eeg_video(args.mat_file, output, args.window, args.fps)
