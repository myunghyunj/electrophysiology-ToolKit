    ...
    if ~isequal(ase_filename, 0)
        try
            ...
            if ~isempty(ase_colors)
                ...
                fprintf('Successfully loaded custom color palette with %d colors for %d z-bins\n', size(ase_colors, 1), num_z_bins);
            else
...
bh_stim = bar(cent, stimStackedData, 'stacked', 'EdgeColor', 'none');
...
ylim([0, maxCount * 1.1]);
% Set y-axis ticks to every 25 units
yticks(0:25:ceil(maxCount * 1.1));
ax1 = gca;
ax1.YColor = 'k';
...
bh_sham = bar(cent, shamStackedData, 'stacked', 'EdgeColor', 'none');
% Set colors for each z-score range
for j = 1:num_z_bins
...
% Set colorbar ticks to every 1.0 sigma for the finite range
tick_values = z_score_min:1:z_score_max;
